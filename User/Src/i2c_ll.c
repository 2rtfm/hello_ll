...
I2C_ReturnVal I2C_MasterTransmit_IT(I2C_Ctx *ctx, uint8_t DevAddr,
  ...
  ctx->left = Size;
  ctx->isDMA = false;
  ctx->status = I2C_STATUS_WAIT_SB_TX;
...
I2C_ReturnVal I2C_MasterReceive_IT(I2C_Ctx *ctx, uint8_t DevAddr,
  ...
  ctx->left = Size;
  ctx->isDMA = false;
  ctx->status = I2C_STATUS_WAIT_SB_RX;
...
void I2C_Handle_EV_IT(I2C_Ctx *ctx) {
  ...
  case I2C_STATUS_IDLE:
    break;
  case I2C_STATUS_WAIT_SB_TX:
    if (LL_I2C_IsActiveFlag_SB(ctx->i2c)) {
      LL_I2C_TransmitData8(ctx->i2c, ctx->addr);
      ctx->status = I2C_STATUS_WAIT_ADDR_TX;
    }
    break;
  case I2C_STATUS_WAIT_ADDR_TX:
    if (LL_I2C_IsActiveFlag_ADDR(ctx->i2c)) {
      LL_I2C_ClearFlag_ADDR(ctx->i2c);
      if (ctx->isDMA) {
        /* 开始 DMA 传输 */
        LL_DMA_SetPeriphAddress(ctx->tx_dma, ctx->tx_dma_channel,
                                LL_I2C_DMA_GetRegAddr(ctx->i2c));
        LL_DMA_SetMemoryAddress(ctx->tx_dma, ctx->tx_dma_channel,
                                (uint32_t)ctx->buf);
        LL_DMA_SetDataLength(ctx->tx_dma, ctx->tx_dma_channel, ctx->left);
        LL_DMA_EnableIT_TC(ctx->tx_dma, ctx->tx_dma_channel);
        LL_I2C_EnableDMAReq_TX(ctx->i2c);
        LL_DMA_EnableChannel(ctx->tx_dma, ctx->tx_dma_channel);
        ctx->status = I2C_STATUS_WAIT_DMA_TC_TX;
      } else {
        LL_I2C_EnableIT_BUF(ctx->i2c);
        ctx->status = I2C_STATUS_WAIT_TXE;
      }
    }
    break;
  case I2C_STATUS_WAIT_TXE:
    if (LL_I2C_IsActiveFlag_TXE(ctx->i2c)) {
      ...
      if (ctx->left == 0U) {
        ...
        ctx->status = I2C_STATUS_WAIT_BTF_TX;
      }
    }
    break;
  case I2C_STATUS_WAIT_BTF_TX:
    if (LL_I2C_IsActiveFlag_BTF(ctx->i2c)) {
      ...
      I2C_MasterTx_Callback(ctx);
    }
    break;
  case I2C_STATUS_WAIT_SB_RX:
    if (LL_I2C_IsActiveFlag_SB(ctx->i2c)) {
      LL_I2C_TransmitData8(ctx->i2c, ctx->addr | 0x01U);
      LL_I2C_AcknowledgeNextData(ctx->i2c, LL_I2C_ACK);
      ctx->status = I2C_STATUS_WAIT_ADDR_RX;
    }
    break;
  case I2C_STATUS_WAIT_ADDR_RX:
    if (LL_I2C_IsActiveFlag_ADDR(ctx->i2c)) {
      if (ctx->left == 1U) {
        /* 单字节：禁止应答 → 清 ADDR → 立即产生停止（EV6_3 时序，
           否则从机会多发一个字节且永远收不到停止） */
        ...
        LL_I2C_GenerateStopCondition(ctx->i2c);
        LL_I2C_EnableIT_BUF(ctx->i2c);
        ctx->status = I2C_STATUS_WAIT_RXNE;
      } else if (ctx->left == 2U) {
        /* 双字节：置 POS=1（让第 1 字节仍被应答）→ 清 ADDR → 禁止应答
           （第 2 字节将收到 NACK，对应 HAL EV6_2 时序） */
        ...
        LL_I2C_ClearFlag_ADDR(ctx->i2c);
        LL_I2C_AcknowledgeNextData(ctx->i2c, LL_I2C_NACK);
        ctx->status = I2C_STATUS_WAIT_BTF_L2_RX;
      } else if (ctx->left == 3U) {
        LL_I2C_ClearFlag_ADDR(ctx->i2c);
        ctx->status = I2C_STATUS_WAIT_BTF_L3_RX;
      } else {
        /* 多字节：使能应答 → 清 ADDR，正常接收 */
        LL_I2C_ClearFlag_ADDR(ctx->i2c);
        if (ctx->isDMA) {
          LL_DMA_SetPeriphAddress(ctx->rx_dma, ctx->rx_dma_channel,
                                  LL_I2C_DMA_GetRegAddr(ctx->i2c));
          LL_DMA_SetMemoryAddress(ctx->rx_dma, ctx->rx_dma_channel,
                                  (uint32_t)ctx->buf);
          LL_DMA_SetDataLength(ctx->rx_dma, ctx->rx_dma_channel, ctx->left - 1);
          LL_DMA_EnableIT_TC(ctx->rx_dma, ctx->rx_dma_channel);
          LL_I2C_EnableDMAReq_RX(ctx->i2c);
          LL_DMA_EnableChannel(ctx->rx_dma, ctx->rx_dma_channel);
          /* DMA 接收时需关闭事件中断避免频繁触发 */
          LL_I2C_DisableIT_EVT(ctx->i2c);
          ctx->status = I2C_STATUS_WAIT_DMA_TC_RX;
        } else {
          LL_I2C_EnableIT_BUF(ctx->i2c);
          ctx->status = I2C_STATUS_WAIT_RXNE;
        }
  ...
  case I2C_STATUS_WAIT_RXNE:
    ...
    break;
  case I2C_STATUS_WAIT_BTF_L2_RX:
    if (LL_I2C_IsActiveFlag_BTF(ctx->i2c)) {
      /* 双字节接收 (NACK 已设置) */
      LL_I2C_GenerateStopCondition(ctx->i2c);
      *ctx->buf++ = LL_I2C_ReceiveData8(ctx->i2c);
    ...
    break;
  case I2C_STATUS_WAIT_BTF_L3_RX:
    if (LL_I2C_IsActiveFlag_BTF(ctx->i2c)) {
    ...
    break;
  case I2C_STATUS_WAIT_BTF_L1_RX:
    if (LL_I2C_IsActiveFlag_BTF(ctx->i2c)) {
      LL_I2C_GenerateStopCondition(ctx->i2c);
      ctx->buf[ctx->left - 1] = LL_I2C_ReceiveData8(ctx->i2c);
      LL_I2C_DisableIT_EVT(ctx->i2c);
      LL_I2C_DisableIT_ERR(ctx->i2c);
      ctx->status = I2C_STATUS_IDLE;
      I2C_MasterRx_Callback(ctx);
...
void I2C_Handle_ER_IT(I2C_Ctx *ctx) {
  ...
  ctx->status = I2C_STATUS_ERROR;
  I2C_Error_Callback(ctx, error);
}
...
static uint32_t I2C_LL_DMA_IsActiveFlag_TC(DMA_TypeDef *dma, uint32_t channel) {
  switch (channel) {
...
static void I2C_LL_DMA_ClearFlag_TC(DMA_TypeDef *dma, uint32_t channel) {
  switch (channel) {
  case LL_DMA_CHANNEL_6:
...
void I2C_Handle_DMA_TX(I2C_Ctx *ctx) {
  if (I2C_LL_DMA_IsActiveFlag_TC(ctx->tx_dma, ctx->tx_dma_channel) &&
      ctx->status == I2C_STATUS_WAIT_DMA_TC_TX) {
    I2C_LL_DMA_ClearFlag_TC(ctx->tx_dma, ctx->tx_dma_channel);
    LL_I2C_DisableDMAReq_TX(ctx->i2c);
...
void I2C_Handle_DMA_RX(I2C_Ctx *ctx) {
  if (I2C_LL_DMA_IsActiveFlag_TC(ctx->rx_dma, ctx->rx_dma_channel) &&
      ctx->status == I2C_STATUS_WAIT_DMA_TC_RX) {
    I2C_LL_DMA_ClearFlag_TC(ctx->rx_dma, ctx->rx_dma_channel);
    LL_I2C_DisableDMAReq_TX(ctx->i2c);
    LL_DMA_DisableChannel(ctx->rx_dma, ctx->rx_dma_channel);
    LL_DMA_DisableIT_TC(ctx->rx_dma, ctx->rx_dma_channel);
    LL_I2C_AcknowledgeNextData(ctx->i2c, LL_I2C_NACK);
    LL_I2C_EnableIT_EVT(ctx->i2c);
    ctx->status = I2C_STATUS_WAIT_BTF_L1_RX;
  }
}
